...
public class LatexContextController {
        ...
        @Operation(summary = "📑 Get LaTeX Context Papers", description = "Retrieve all papers that are marked as LaTeX context for a specific project. "
                        + "These papers will be automatically loaded in the LaTeX editor for the project.")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "LaTeX context papers retrieved successfully", content = @Content(schema = @Schema(implementation = APIResponse.class))),
                        @ApiResponse(responseCode = "404", description = "Project not found")
        })
        public ResponseEntity<APIResponse<List<PaperMetadataDto>>> getLatexContextPapers(
                        @PathVariable @Parameter(description = "Project ID to retrieve LaTeX context papers for", example = "123e4567-e89b-12d3-a456-426614174000") UUID projectId) {
                try {
                        log.info("Retrieving LaTeX context papers for project: {}", projectId);

                        List<PaperMetadataDto> contextPapers = paperPersistenceService
                                        .findLatexContextPapersByProjectId(projectId);

                        return ResponseEntity.ok(APIResponse.success(
                                        HttpStatus.OK.value(),
                                        String.format("Retrieved %d LaTeX context papers for project",
                                                        contextPapers.size()),
                                        contextPapers));
                } catch (Exception e) {
                        log.error("Error retrieving LaTeX context papers for project {}: {}", projectId,
                                        e.getMessage());
        ...
        @Operation(summary = "🔄 Toggle LaTeX Context Status", description = "Toggle whether a paper is included in LaTeX context for its project. "
                        + "Papers marked as LaTeX context will be automatically loaded in the LaTeX editor.")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "LaTeX context status updated successfully", content = @Content(schema = @Schema(implementation = APIResponse.class))),
        ...
        public ResponseEntity<APIResponse<PaperMetadataDto>> toggleLatexContext(
                        @PathVariable @Parameter(description = "Paper ID to toggle LaTeX context status", example = "123e4567-e89b-12d3-a456-426614174000") UUID paperId,
                ...
                try {
                        ...
                        PaperMetadataDto updatedPaper = paperPersistenceService.toggleLatexContext(paperId,
                                        request.isLatexContext());

                        String action = request.isLatexContext() ? "added to" : "removed from";
                        return ResponseEntity.ok(APIResponse.success(
                                        HttpStatus.OK.value(),
                                        String.format("Paper successfully %s LaTeX context", action), updatedPaper));
        ...
        public record ToggleLatexContextRequest(
                        @Schema(description = "Whether to include this paper in LaTeX context", example = "true") boolean isLatexContext) {
        }
}
