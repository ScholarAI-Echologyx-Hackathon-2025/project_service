...
public class ExtractionController {
         ...
         *         status
         */
        @Operation(summary = "Trigger paper extraction", description = "Triggers content extraction for a paper using its PDF content URL")
        @ApiResponses(value = {
         ...
         *         information
         */
        @Operation(summary = "Get extraction status", description = "Gets the current extraction status and progress for a paper")
        @ApiResponses(value = {
         ...
         */
        @Operation(summary = "Get extraction status only", description = "Gets only the extraction status string for a paper")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Status retrieved successfully"),
                        @ApiResponse(responseCode = "404", description = "Paper not found"),
                        @ApiResponse(responseCode = "500", description = "Internal server error")
         ...
         *         status
         */
        @Operation(summary = "Trigger extraction with default options", description = "Triggers content extraction for a paper with default extraction options")
        @ApiResponses(value = {
                        @ApiResponse(responseCode = "200", description = "Extraction triggered successfully"),
                        @ApiResponse(responseCode = "400", description = "Invalid request or paper has no PDF URL"),
                        @ApiResponse(responseCode = "404", description = "Paper not found"),
        ...
        public ResponseEntity<ExtractionResponse> triggerExtractionForPaper(
                        @Parameter(description = "Paper ID", required = true) @PathVariable String paperId,
                        @Parameter(description = "Process asynchronously") @RequestParam(defaultValue = "true") Boolean asyncProcessing) {

                log.info("Received extraction request for paper ID: {} (async: {})", paperId, asyncProcessing);

                ExtractionRequest request = new ExtractionRequest(
                                paperId, true, true, true, true, true, true, true, true, asyncProcessing);
         ...
         */
        @Operation(summary = "Check if paper is extracted", description = "Returns a boolean indicating whether the paper content has been successfully extracted")
        @ApiResponses(value = {
         ...
         */
        @Operation(summary = "Get extracted figures for a paper", description = "Retrieves all extracted figures for a paper including image path, caption, and page number")
        @ApiResponses(value = {
         ...
         */
        @Operation(summary = "Get extracted tables for a paper", description = "Retrieves all extracted tables for a paper including CSV path, caption, and page number")
        @ApiResponses(value = {
